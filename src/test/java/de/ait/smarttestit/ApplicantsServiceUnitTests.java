...
import de.ait.smarttestit.services.impl.ApplicantsServiceImpl;
...
public class ApplicantsServiceUnitTests {
    ...
    private static final Applicant APPLICANT = new Applicant(APPLICANT_ID, "Applicant", "Applicantov",
            "applicantov@mail.com", "123 Main St", "555-1234", "hashedPassword123", EXAMS);

    private static final NewApplicantDto NEW_APPLICANT_DTO = new NewApplicantDto("Applicant", "Applicantov",
            "applicantov@mail.com", "123 Main St", "555-1234", "hashedPassword123");

    @Mock
    private ApplicantsRepository applicantsRepository;

    @InjectMocks
    private ApplicantsServiceImpl applicantsServices;
    ...
    class GetApplicantOrThrow {
        ...
        void getExamOrThrowPositive() {
            ...
            when(applicantsRepository.findById(APPLICANT_ID)).thenReturn(Optional.of(APPLICANT));
        ...
        void getExamOrThrowNegative() {
            when(applicantsRepository.findById(APPLICANT_ID)).thenReturn(Optional.empty());
    ...
    class AddApplicant {
        ...
        void testAddApplicantPositive() {
            ...
            when(applicantsRepository.existsByEmail(anyString())).thenReturn(false);
            when(applicantsRepository.save(any(Applicant.class))).thenReturn(APPLICANT);

            ApplicantDto result = applicantsServices.addApplicant(NEW_APPLICANT_DTO);

            assertNotNull(result);
            assertEquals(APPLICANT.getId(), result.getId());
            verify(applicantsRepository, times(1)).save(any(Applicant.class));
        }

        @Test
        void testAddApplicantNegative() {

            when(applicantsRepository.existsByEmail(anyString())).thenReturn(true);
    ...
    class GetListApplicants {
        ...
        void testGetListApplicantsPositive() {
            ...
            Applicant applicant1 = new Applicant(1L, "Ivan", "Ivanov", "ivan@example.com", "hash1", "Address 1", "1234567890");
            Applicant applicant2 = new Applicant(2L, "Petr", "Petrov", "petr@example.com", "hash2", "Address 2", "0987654321");
            when(applicantsRepository.findAll()).thenReturn(Arrays.asList(applicant1, applicant2));
        ...
        void testGetListApplicantsNegative() {
            ...
            when(applicantsRepository.findAll()).thenReturn(Collections.emptyList());
    ...
    class deleteApplicant {
        ...
        void testDeleteApplicantPositive() {
            ...
            when(applicantsRepository.findById(APPLICANT_ID)).thenReturn(Optional.of(APPLICANT));
            doNothing().when(applicantsRepository).delete(any(Applicant.class));

            ApplicantDto result = applicantsServices.deleteApplicant(APPLICANT_ID);

            assertNotNull(result);
            assertEquals(APPLICANT_ID, result.getId());
            verify(applicantsRepository).delete(APPLICANT);
        }
        ...
        void testDeleteApplicantNegative() {
            ...
            when(applicantsRepository.findById(invalidApplicantId)).thenReturn(Optional.empty());

            assertThrows(RestException.class, () -> applicantsServices.deleteApplicant(invalidApplicantId));

            verify(applicantsRepository, never()).delete(any(Applicant.class));
        }
    }
    ...
    class updateApplicant {
        ...
        void testUpdateApplicantPositive() {
            ...
            UpdateApplicantDto updateDto = new UpdateApplicantDto("UpdatedFirstName", "UpdatedLastName",
                    "updated@example.com", "passwordHash", "updatedOriginalAddress", "+1234567890");
            Applicant existingApplicant = new Applicant(applicantId, "OriginalFirstName", "OriginalLastName",
                    "original@example.com", "originalAddress", "+0987654321", "passwordHash");
            when(applicantsRepository.findById(applicantId)).thenReturn(Optional.of(existingApplicant));
            when(applicantsRepository.save(any(Applicant.class))).thenAnswer(invocation -> invocation.getArgument(0));

            ApplicantDto result = applicantsServices.updateApplicant(applicantId, updateDto);

            assertEquals("UpdatedFirstName", result.getFirstName());
            assertEquals("updated@example.com", result.getEmail());
            verify(applicantsRepository).save(any(Applicant.class));
        }
        ...
        void testUpdateApplicantNegative() {
            ...
            UpdateApplicantDto updateDto = new UpdateApplicantDto("UpdatedFirstName", "UpdatedLastName",
                    "updated@example.com", "passwordHash", "updatedOriginalAddress", "+1234567890");
            when(applicantsRepository.findById(nonExistentApplicantId)).thenReturn(Optional.empty());

            assertThrows(RestException.class, () -> applicantsServices.updateApplicant(nonExistentApplicantId, updateDto));
            verify(applicantsRepository, never()).save(any(Applicant.class));
        }
    }
